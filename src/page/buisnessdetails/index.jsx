...
const BuisnessDetails = () => {
  ...
  return (
    <div className='w-full'>
        ...
        <div>
        <form className='flex w-full gap-4 px-10 overflo mob:flex-col-reverse' onSubmit={socialSubmit}>
                
            {
              socialName.map((items, index) => {
                return   <div className='flex min-w-[30vw] flex-col w-1/2 gap-4 mob:w-full'>
              <div className='flex justify-between w-full'>
                <Text className="text-[20px] font-semibold">Social Media Login Details</Text>
                <div className='flex gap-4'>

                <label onClick={socialAppend}  className='flex px-3 py-2 cursor-pointer rounded-lg bg-[#45818E] text-white w-fit' >
                  <Add  className='!text-white'/>
                    Add
                  </label>
                 {index > 0 && <label onClick={socialRemove} className='flex cursor-pointer px-3 py-2 rounded-lg bg-[#DC143C] text-white w-fit' >
                ...
                <select className='common-select' {...socialRegister(`social${index}Name`)} placeholder="Category">
                  <option>category1</option>
                  <option>category1</option>
                  <option>category1</option>
                  <option>category1</option>
                </select>
                <input {...socialRegister(`social${index}userId`)} placeholder='User ID' className='common-input'  {...register("userId")}/>
                <input {...socialRegister(`social${index}password`)} placeholder='password' className='common-input'  {...register("password")}/>
                </div>
